...
<div class="product">
    <div class="container">
        <div class="details">
            ...
            <p>السعر: <span> <?php echo $row['price'] ?> L.E</span></p>
            <p>متاح : <span> <?php echo $row['quantity'] ?> قطعة</span></p>
<form action="add_cart.php" method="post">
    

    <?php if ($row['quantity'] == 0): ?>
        <label for="">ناسف لعدم تواجد المنتج  </label>
        <button type="submit" class="btn btn-success" name="add_to_cart" disabled>اضف الي السلة</button>
    <?php else: ?>
        <label for="">اختر الجودة</label>
        <input type="number" name="quantity" class="m-2" value="1" min="1" max="<?php echo $row['quantity'] ?>" required>
    <input type="hidden" name="product_id" value="<?php echo $row['id'] ?>">
        <button type="submit" class="btn btn-warning" name="add_to_cart">اضف الي السلة</button>
    <?php endif; ?>
</form>
            <div class="social-media">
                <h3>المشاركة عبر وسائل التواصل الاجتماعي</h3>
                <ul>
